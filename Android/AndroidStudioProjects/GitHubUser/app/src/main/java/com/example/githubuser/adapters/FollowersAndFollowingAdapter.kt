...
class FollowersAndFollowingAdapter(parentFragment: Fragment) : FragmentStateAdapter(parentFragment) {
    override fun getItemCount(): Int = FollowType.values().size
    override fun createFragment(position: Int): Fragment = FollowersAndFollowingListFragment(FollowType.values()[position])
}